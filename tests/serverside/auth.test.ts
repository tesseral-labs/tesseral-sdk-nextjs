import { __exportedForTests as __configExportedForTests } from "../../src/internal/common/config";
import { __exportedForTests } from "../../src/internal/serverside/auth";
...
describe("authenticate", () => {
  for (const testCase of ACCESS_TOKEN_TEST_CASES) {
    it(testCase.name, async () => {
      const { jwks } = await __configExportedForTests.parseConfig(JSON.parse(testCase.jwks));
...
describe("base64URLDecode", () => {
  ...
  expect(() => __exportedForTests.base64URLDecode(claims)).not.toThrow();
});
