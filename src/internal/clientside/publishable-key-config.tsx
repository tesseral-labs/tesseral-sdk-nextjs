    ...
    (async () => {
      try {
        ...
        if (response.status === 400 || response.status === 404) {
          throw new Error(
            `Tesseral Publishable Key ${configApiHostname} not found. Go to https://console.tesseral.com/settings/api-keys to see your list of Publishable Keys, and then update your <TesseralProvider publishableKey={...} /> call to use one of those keys.`,
          );
        }
        if (!response.ok) {
          throw new Error(
